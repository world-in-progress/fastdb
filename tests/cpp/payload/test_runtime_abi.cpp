...
constexpr std::string_view kPublicGraphSpec = R"({
  "schema":"fastdb.payload.v1",
  "profile":"object_graph.v1",
  "entries":[
...
static_assert(sizeof(fdb_payload_v1_object_handle_t) == sizeof(std::uint64_t));
static_assert(FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE == UINT64_C(0));
...
int test_initializers_and_constants() {
    ...
    require(builder.struct_size == FDB_PAYLOAD_V1_BUILDER_OPTIONS_V2_SIZE);
    ...
    require(builder.max_graph_objects == UINT64_C(10000000));
    ...
    require(info.struct_size == FDB_PAYLOAD_V1_PLAN_INFO_V2_SIZE);
    require(info.graph_object_count == UINT64_C(0));
...
void require_abi_success(fdb_payload_v1_status_t status,
                         const ErrorRef& error) {
    if (status != UINT32_C(0) || error.value != nullptr) {
        throw std::runtime_error("unexpected public ABI status " +
                                 std::to_string(status) + " " +
                                 std::string(error_details(error.value)));
    }
}

void author_public_graph(fdb_payload_v1_builder_t* builder,
                         std::uint32_t node_component,
                         std::uint32_t asset_component,
                         ErrorRef& error) {
    fdb_payload_v1_object_handle_t node =
        FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE;
    fdb_payload_v1_object_handle_t asset =
        FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE;
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            builder, node_component, &node, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            builder, asset_component, &asset, &error.value),
                        error);
    if (node == FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE ||
        asset == FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE || node == asset) {
        throw std::runtime_error("invalid public graph object token");
    }

    static constexpr std::array<std::uint16_t, 3> wide{{
        UINT16_C(0x0041), UINT16_C(0xd83d), UINT16_C(0xde00)}};
    static constexpr std::array<std::uint8_t, 3> opaque{{
        UINT8_C(0x00), UINT8_C(0xff), UINT8_C(0x7e)}};
    static constexpr std::array<std::uint8_t, 4> text{{'s', 'a', 'm', 'e'}};

    require_abi_success(fdb_payload_v1_builder_object_fill_begin(
                            builder, node, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_bool(
                            builder, UINT8_C(1), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u8(
                            builder, UINT8_C(0x12), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u16(
                            builder, UINT16_C(0x3456), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u32(
                            builder, UINT32_C(0x789abcde), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_i32(
                            builder, INT32_C(-1234567), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u8n_f64_bits(
                            builder, UINT64_C(0x3fe0000000000000),
                            &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u16n_f64_bits(
                            builder, UINT64_C(0), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_f32_bits(
                            builder, UINT32_C(0x7fa12345), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_f64_bits(
                            builder, UINT64_C(0xfff8000000001234),
                            &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_str(
                            builder, text.data(), text.size(), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_wstr(
                            builder, wide.data(), wide.size(), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_bytes(
                            builder, opaque.data(), opaque.size(),
                            &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_component_begin(
                            builder, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_null(
                            builder, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u16(
                            builder, UINT16_C(0xbeef), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_list_begin(
                            builder, UINT64_C(3), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_f32_bits(
                            builder, UINT32_C(0x80000000), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_null(
                            builder, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_f32_bits(
                            builder, UINT32_C(0xff800001), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_ref(
                            builder, node, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_ref(
                            builder, asset, &error.value),
                        error);
    ...
    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            builder, UINT32_C(0), UINT64_C(1), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_object(
                            builder, node, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            builder, UINT32_C(1), UINT64_C(1), &error.value),
    ...
    require_abi_success(fdb_payload_v1_builder_value_object(
                        ...
                        error);
    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            builder, UINT32_C(2), UINT64_C(2), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_ref(
                            builder, node, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_null(
                        ...
                        error);
    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            builder, UINT32_C(3), UINT64_C(3), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u8n_f64_bits(
                            builder, UINT64_C(0), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u8n_f64_bits(
                        ...
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u8n_f64_bits(
                            builder, UINT64_C(0x3ff0000000000000),
                            &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            builder, UINT32_C(4), UINT64_C(3), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u16n_f64_bits(
                            builder, UINT64_C(0xbff0000000000000),
                            &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_u16n_f64_bits(
                            builder, UINT64_C(0), &error.value),
...
int test_graph_builder_option_prefixes() {
    fdb_payload_v1_spec_t* spec = compile_spec(kPublicGraphSpec);
    require(spec != nullptr);
    ErrorRef error;

    fdb_payload_v1_builder_options_t short_options{};
    fdb_payload_v1_builder_options_init(&short_options);
    short_options.struct_size = FDB_PAYLOAD_V1_BUILDER_OPTIONS_V1_SIZE - 1U;
    fdb_payload_v1_builder_t* builder =
        reinterpret_cast<fdb_payload_v1_builder_t*>(std::uintptr_t{1});
    auto status = fdb_payload_v1_builder_create(
        spec, &short_options, &builder, &error.value);
    require(status == FDB_PAYLOAD_E_UNSUPPORTED_ABI);
    require(builder == nullptr && owned_error(status, error.value));
    error.clear();

    const auto accepts_two_objects = [&](std::uint32_t size,
                                         std::uint64_t graph_limit) -> int {
        fdb_payload_v1_builder_options_t options{};
        fdb_payload_v1_builder_options_init(&options);
        options.struct_size = size;
        options.max_graph_objects = graph_limit;
        fdb_payload_v1_builder_t* local = nullptr;
        require_abi_success(fdb_payload_v1_builder_create(
                                spec, &options, &local, &error.value),
                            error);
        fdb_payload_v1_object_handle_t first = UINT64_C(0);
        fdb_payload_v1_object_handle_t second = UINT64_C(0);
        require_abi_success(fdb_payload_v1_builder_object_declare(
                                local, UINT32_C(0), &first, &error.value),
                            error);
        require_abi_success(fdb_payload_v1_builder_object_declare(
                                local, UINT32_C(0), &second, &error.value),
                            error);
        require(first != UINT64_C(0) && second != UINT64_C(0));
        fdb_payload_v1_builder_release(local);
        return EXIT_SUCCESS;
    };
    require(accepts_two_objects(FDB_PAYLOAD_V1_BUILDER_OPTIONS_V1_SIZE,
                                UINT64_C(1)) == EXIT_SUCCESS);
    for (std::uint32_t size = FDB_PAYLOAD_V1_BUILDER_OPTIONS_V1_SIZE + 1U;
         size < FDB_PAYLOAD_V1_BUILDER_OPTIONS_V2_SIZE; ++size) {
        require(accepts_two_objects(size, UINT64_C(1)) == EXIT_SUCCESS);
    }
    require(accepts_two_objects(FDB_PAYLOAD_V1_BUILDER_OPTIONS_V2_SIZE,
                                UINT64_C(0)) == EXIT_SUCCESS);

    fdb_payload_v1_builder_options_t limited{};
    fdb_payload_v1_builder_options_init(&limited);
    limited.max_graph_objects = UINT64_C(1);
    builder = nullptr;
    require_abi_success(fdb_payload_v1_builder_create(
                            spec, &limited, &builder, &error.value),
                        error);
    fdb_payload_v1_object_handle_t object = UINT64_C(0);
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            builder, UINT32_C(0), &object, &error.value),
                        error);
    object = UINT64_MAX;
    status = fdb_payload_v1_builder_object_declare(
        builder, UINT32_C(0), &object, &error.value);
    require(status == FDB_PAYLOAD_E_BUILDER_RESOURCE_LIMIT);
    require(object == FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE);
    require(owned_error(status, error.value));
    error.clear();
    ...
    future.known.max_graph_objects = UINT64_C(2);
    future.tail.fill(UINT8_C(0xa5));
    builder = nullptr;
    require_abi_success(fdb_payload_v1_builder_create(
                            spec, &future.known, &builder, &error.value),
                        error);
    ...
    builder = nullptr;
    require_abi_success(fdb_payload_v1_builder_create(
                            spec, nullptr, &builder, &error.value),
                        error);
    object = UINT64_C(0xfeed);
    require(fdb_payload_v1_builder_object_declare(
                builder, UINT32_C(0), &object, nullptr) ==
            FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(object == UINT64_C(0xfeed));
    object = UINT64_MAX;
    status = fdb_payload_v1_builder_object_declare(
        nullptr, UINT32_C(0), &object, &error.value);
    ...
    for (std::int64_t allocation = INT64_C(0); allocation < INT64_C(4096);
        ...
        if (status == UINT32_C(0)) {
            require(object != FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE);
            require(error.value == nullptr && saw_allocation_failure);
            declared = true;
            break;
        }
        require(status == FDB_PAYLOAD_E_ALLOCATION_FAILED);
...
#if (defined(__unix__) || defined(__APPLE__)) && !defined(__EMSCRIPTEN__)
int test_guarded_v1_prefixes_do_not_touch_v2_tails() {
    ...
    require(page_size % alignof(fdb_payload_v1_builder_options_t) == 0U);
    require(page_size % alignof(fdb_payload_v1_plan_info_t) == 0U);
    void* const pages =
        ::mmap(nullptr, page_size * 2U, PROT_READ | PROT_WRITE,
               MAP_PRIVATE | MAP_ANON, -1, 0);
    require(pages != MAP_FAILED);
    auto* const protected_page = static_cast<std::uint8_t*>(pages) + page_size;
    require(::mprotect(protected_page, page_size, PROT_NONE) == 0);

    fdb_payload_v1_spec_t* spec = compile_spec(kObjectGraphSpec);
    require(spec != nullptr);
    ErrorRef error;

    fdb_payload_v1_builder_options_t initialized{};
    fdb_payload_v1_builder_options_init(&initialized);
    auto* const options_bytes =
        protected_page - FDB_PAYLOAD_V1_BUILDER_OPTIONS_V1_SIZE;
    require(reinterpret_cast<std::uintptr_t>(options_bytes) %
                alignof(fdb_payload_v1_builder_options_t) ==
            0U);
    std::memcpy(options_bytes, &initialized,
                FDB_PAYLOAD_V1_BUILDER_OPTIONS_V1_SIZE);
    auto* const old_options =
        reinterpret_cast<fdb_payload_v1_builder_options_t*>(options_bytes);
    old_options->struct_size = FDB_PAYLOAD_V1_BUILDER_OPTIONS_V1_SIZE;
    fdb_payload_v1_builder_t* old_builder = nullptr;
    require_abi_success(fdb_payload_v1_builder_create(
                            spec, old_options, &old_builder, &error.value),
                        error);
    fdb_payload_v1_object_handle_t first = UINT64_C(0);
    fdb_payload_v1_object_handle_t second = UINT64_C(0);
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            old_builder, UINT32_C(0), &first, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            old_builder, UINT32_C(0), &second, &error.value),
                        error);
    require(first != FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE &&
            second != FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE);
    fdb_payload_v1_builder_release(old_builder);
    ...
    require_abi_success(fdb_payload_v1_builder_object_declare(
                        ...
                        error);
    require_abi_success(fdb_payload_v1_builder_object_fill_begin(
                            builder, object, &error.value),
                        error);
    static constexpr std::array<std::uint8_t, 1> label{{'x'}};
    require_abi_success(fdb_payload_v1_builder_value_str(
                            builder, label.data(), label.size(), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_null(
                            builder, &error.value),
                        error);
    ...
    require_abi_success(fdb_payload_v1_builder_freeze(
                            builder, &plan, &error.value),
                        error);
    fdb_payload_v1_builder_release(builder);

    auto* const info_bytes =
        protected_page - FDB_PAYLOAD_V1_PLAN_INFO_V1_SIZE;
    require(reinterpret_cast<std::uintptr_t>(info_bytes) %
                alignof(fdb_payload_v1_plan_info_t) ==
            0U);
    std::memset(info_bytes, 0, FDB_PAYLOAD_V1_PLAN_INFO_V1_SIZE);
    auto* const old_info =
        reinterpret_cast<fdb_payload_v1_plan_info_t*>(info_bytes);
    old_info->struct_size = FDB_PAYLOAD_V1_PLAN_INFO_V1_SIZE;
    require_abi_success(
        fdb_payload_v1_plan_info(plan, old_info, &error.value), error);
    require(old_info->total_bytes != UINT64_C(0));

    fdb_payload_v1_plan_release(plan);
    fdb_payload_v1_spec_release(spec);
    require(::mprotect(protected_page, page_size,
...
int test_public_graph_handle_errors() {
    ErrorRef error;
    fdb_payload_v1_spec_t* record = compile_spec(kRecordSpec);
    require(record != nullptr);
    fdb_payload_v1_builder_t* record_builder = nullptr;
    require_abi_success(fdb_payload_v1_builder_create(
                            record, nullptr, &record_builder, &error.value),
                        error);
    fdb_payload_v1_object_handle_t object = UINT64_MAX;
    auto status = fdb_payload_v1_builder_object_declare(
        record_builder, UINT32_C(0), &object, &error.value);
    require(status == FDB_PAYLOAD_E_PROFILE_VIOLATION);
    require(object == FDB_PAYLOAD_V1_INVALID_OBJECT_HANDLE);
    require(owned_error(status, error.value));
    error.clear();
    object = UINT64_MAX;
    status = fdb_payload_v1_builder_object_declare(
        record_builder, UINT32_C(0), nullptr, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(owned_error(status, error.value));
    error.clear();
    fdb_payload_v1_builder_release(record_builder);
    fdb_payload_v1_spec_release(record);

    fdb_payload_v1_spec_t* graph = compile_spec(kObjectGraphSpec);
    require(graph != nullptr);
    std::uint32_t node_component = UINT32_MAX;
    static constexpr std::array<std::uint8_t, 4> node_id{{'N', 'o', 'd', 'e'}};
    require_abi_success(fdb_payload_v1_spec_component_index(
                            graph, node_id.data(), node_id.size(),
                            &node_component, &error.value),
                        error);
    fdb_payload_v1_builder_t* first = nullptr;
    fdb_payload_v1_builder_t* second = nullptr;
    require_abi_success(fdb_payload_v1_builder_create(
                            graph, nullptr, &first, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_create(
                            graph, nullptr, &second, &error.value),
                        error);
    fdb_payload_v1_object_handle_t first_object = UINT64_C(0);
    fdb_payload_v1_object_handle_t second_object = UINT64_C(0);
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            first, node_component, &first_object,
                            &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_object_declare(
                            second, node_component, &second_object,
                            &error.value),
                        error);

    constexpr fdb_payload_v1_object_handle_t forged =
        UINT64_C(0x1122334455667788);
    status = fdb_payload_v1_builder_object_fill_begin(
        second, forged, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_OBJECT_HANDLE);
    require(owned_error(status, error.value));
    require(error_details(error.value).find(std::to_string(forged)) ==
            std::string_view::npos);
    error.clear();
    status = fdb_payload_v1_builder_object_fill_begin(
        second, first_object, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_OBJECT_HANDLE);
    require(owned_error(status, error.value));
    error.clear();

    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            second, UINT32_C(0), UINT64_C(1), &error.value),
                        error);
    status = fdb_payload_v1_builder_value_ref(
        second, first_object, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_OBJECT_HANDLE);
    require(owned_error(status, error.value));
    error.clear();
    status = fdb_payload_v1_builder_value_object(
        second, second_object, &error.value);
    require(status == FDB_PAYLOAD_E_TYPE_MISMATCH);
    require(owned_error(status, error.value));
    error.clear();

    require_abi_success(fdb_payload_v1_builder_object_fill_begin(
                            first, first_object, &error.value),
                        error);
    static constexpr std::array<std::uint8_t, 1> label{{'x'}};
    require_abi_success(fdb_payload_v1_builder_value_str(
                            first, label.data(), label.size(), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_null(
                            first, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_entry_begin(
                            first, UINT32_C(0), UINT64_C(1), &error.value),
                        error);
    require_abi_success(fdb_payload_v1_builder_value_ref(
                            first, first_object, &error.value),
                        error);
    fdb_payload_v1_plan_t* plan = nullptr;
    require_abi_success(fdb_payload_v1_builder_freeze(
                            first, &plan, &error.value),
                        error);
    status = fdb_payload_v1_builder_object_fill_begin(
        first, first_object, &error.value);
    require(status == FDB_PAYLOAD_E_BUILDER_STATE);
    require(owned_error(status, error.value));
    error.clear();

    fdb_payload_v1_plan_release(plan);
    fdb_payload_v1_builder_release(first);
    fdb_payload_v1_builder_release(second);
...
int test_complete_record_runtime() {
    ...
    auto graph_open_status = fdb_payload_v1_payload_open_copy(
        ...
        &error.value);
    require(graph_open_status == FDB_PAYLOAD_E_NON_CANONICAL_BINARY,
...
int test_complete_public_graph_runtime() {
    fdb_payload_v1_spec_t* spec = compile_spec(kPublicGraphSpec);
    require(spec != nullptr);
    ErrorRef error;
    fdb_payload_v1_builder_t* builder = nullptr;
    ...
    static constexpr std::array<std::uint8_t, 4> node_id{{'N', 'o', 'd', 'e'}};
    static constexpr std::array<std::uint8_t, 5> asset_id{{
        'A', 's', 's', 'e', 't'}};
    static constexpr std::array<std::uint8_t, 6> inline_id{{
        'I', 'n', 'l', 'i', 'n', 'e'}};
    std::uint32_t node_component = UINT32_MAX;
    std::uint32_t asset_component = UINT32_MAX;
    std::uint32_t inline_component_index = UINT32_MAX;
    require_abi_success(fdb_payload_v1_spec_component_index(
                            spec, node_id.data(), node_id.size(),
                            &node_component, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_spec_component_index(
                            spec, asset_id.data(), asset_id.size(),
                            &asset_component, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_spec_component_index(
                            spec, inline_id.data(), inline_id.size(),
                            &inline_component_index, &error.value),
                        error);
    author_public_graph(builder, node_component, asset_component, error);
    ...
    fdb_payload_v1_plan_info_init(&info);
    require_abi_success(
        fdb_payload_v1_plan_info(plan, &info, &error.value), error);
    require(info.struct_size == FDB_PAYLOAD_V1_PLAN_INFO_V2_SIZE);
    require(info.graph_object_count == UINT64_C(2));
    require(info.direct_build_status == FDB_PAYLOAD_DIRECT_BUILD_ELIGIBLE);

    fdb_payload_v1_plan_info_t short_info{};
    std::memset(&short_info, 0xa5, sizeof(short_info));
    short_info.struct_size = FDB_PAYLOAD_V1_PLAN_INFO_V1_SIZE - UINT32_C(8);
    auto status =
        fdb_payload_v1_plan_info(plan, &short_info, &error.value);
    require(status == FDB_PAYLOAD_E_UNSUPPORTED_ABI);
    require(owned_error(status, error.value));
    const auto* short_bytes =
        reinterpret_cast<const std::uint8_t*>(&short_info);
    for (std::uint32_t index = UINT32_C(4);
         index < short_info.struct_size; ++index) {
        require(short_bytes[index] == UINT8_C(0));
    }
    require(short_bytes[short_info.struct_size] == UINT8_C(0xa5));
    error.clear();

    for (std::uint32_t declared = FDB_PAYLOAD_V1_PLAN_INFO_V1_SIZE;
         declared < FDB_PAYLOAD_V1_PLAN_INFO_V2_SIZE; ++declared) {
        fdb_payload_v1_plan_info_t prefix{};
        std::memset(&prefix, 0xa5, sizeof(prefix));
        prefix.struct_size = declared;
        prefix.flags = UINT32_C(0);
        std::fill(std::begin(prefix.reserved), std::end(prefix.reserved),
                  UINT64_C(0));
        require_abi_success(
            fdb_payload_v1_plan_info(plan, &prefix, &error.value), error);
        require(prefix.total_bytes == info.total_bytes);
        const auto* prefix_bytes =
            reinterpret_cast<const std::uint8_t*>(&prefix);
        for (std::uint32_t index = FDB_PAYLOAD_V1_PLAN_INFO_V1_SIZE;
             index < FDB_PAYLOAD_V1_PLAN_INFO_V2_SIZE; ++index) {
            require(prefix_bytes[index] == UINT8_C(0xa5));
        }
    }

    struct FuturePlanInfo final {
        fdb_payload_v1_plan_info_t known;
        std::array<std::uint8_t, 16> tail;
    } future_info{};
    fdb_payload_v1_plan_info_init(&future_info.known);
    future_info.known.struct_size = sizeof(future_info);
    future_info.tail.fill(UINT8_C(0xa5));
    require_abi_success(fdb_payload_v1_plan_info(
                            plan, &future_info.known, &error.value),
                        error);
    require(future_info.known.graph_object_count == UINT64_C(2));
    require(std::all_of(future_info.tail.begin(), future_info.tail.end(),
                        [](std::uint8_t value) {
                            return value == UINT8_C(0xa5);
                        }));

    fdb_payload_v1_payload_t* heap_payload = nullptr;
    fdb_payload_v1_execution_report_t heap_report{};
    fdb_payload_v1_execution_report_init(&heap_report);
    require_abi_success(fdb_payload_v1_plan_execute(
                            plan, FDB_PAYLOAD_BUILD_ALLOW_STAGING, nullptr,
                            &heap_payload, &heap_report, &error.value),
                        error);
    require(heap_payload != nullptr);
    require(heap_report.mode == FDB_PAYLOAD_EXECUTION_DIRECT);
    fdb_payload_v1_blob_t* binary = nullptr;
    require_abi_success(fdb_payload_v1_payload_binary_blob(
                            heap_payload, &binary, &error.value),
                        error);
    require(binary != nullptr);
    require(fdb_payload_v1_blob_size(binary) == info.total_bytes);

    const auto same_as_binary = [&](fdb_payload_v1_payload_t* payload) -> int {
        fdb_payload_v1_blob_t* candidate = nullptr;
        require_abi_success(fdb_payload_v1_payload_binary_blob(
                                payload, &candidate, &error.value),
                            error);
        require(candidate != nullptr);
        require(fdb_payload_v1_blob_size(candidate) ==
                fdb_payload_v1_blob_size(binary));
        require(std::memcmp(fdb_payload_v1_blob_data(candidate),
                            fdb_payload_v1_blob_data(binary),
                            static_cast<std::size_t>(
                                fdb_payload_v1_blob_size(binary))) == 0);
        fdb_payload_v1_blob_release(candidate);
        return EXIT_SUCCESS;
    };
    ...
    require_abi_success(fdb_payload_v1_plan_execute(
                            plan, FDB_PAYLOAD_BUILD_REQUIRE_DIRECT,
                            &direct_backing, &direct_payload, &direct_report,
                            &error.value),
                        error);
    require(direct_report.mode == FDB_PAYLOAD_EXECUTION_DIRECT);
    require(same_as_binary(direct_payload) == EXIT_SUCCESS);
    fdb_payload_v1_payload_release(direct_payload);
    require(direct_context.releases == UINT32_C(1));
    ...
    require_abi_success(fdb_payload_v1_plan_execute(
                            ...
                            &staged_backing, &staged_payload, &staged_report,
                            &error.value),
                        error);
    require(staged_report.mode == FDB_PAYLOAD_EXECUTION_STAGED);
    require(staged_report.fallback_reason ==
            FDB_PAYLOAD_FALLBACK_BACKING_DECLINED_DIRECT);
    require(same_as_binary(staged_payload) == EXIT_SUCCESS);
    fdb_payload_v1_payload_release(staged_payload);
    require(staged_context.releases == UINT32_C(1));

    fdb_payload_v1_payload_t* opened = nullptr;
    require_abi_success(fdb_payload_v1_payload_open_copy(
                            spec, fdb_payload_v1_blob_data(binary),
                            fdb_payload_v1_blob_size(binary), nullptr, &opened,
                            &error.value),
                        error);
    require(opened != nullptr);
    fdb_payload_v1_profile_t profile = UINT32_C(0);
    require_abi_success(fdb_payload_v1_payload_profile(
                            opened, &profile, &error.value),
                        error);
    require(profile == FDB_PAYLOAD_PROFILE_OBJECT_GRAPH_V1);

    const auto entry_value = [&](fdb_payload_v1_payload_t* payload,
                                 std::uint32_t entry,
                                 std::uint64_t index) {
        fdb_payload_v1_view_t* sequence = nullptr;
        require_abi_success(fdb_payload_v1_payload_entry_view(
                                payload, entry, &sequence, &error.value),
                            error);
        fdb_payload_v1_view_t* value = nullptr;
        require_abi_success(fdb_payload_v1_view_at(
                                sequence, index, &value, &error.value),
                            error);
        fdb_payload_v1_view_release(sequence);
        if (value == nullptr) {
            throw std::runtime_error("public graph entry view is null");
        }
        return value;
    };
    const auto field = [&](const fdb_payload_v1_view_t* parent,
                           std::uint32_t index) {
        fdb_payload_v1_view_t* value = nullptr;
        require_abi_success(fdb_payload_v1_view_field(
                                parent, index, &value, &error.value),
                            error);
        if (value == nullptr) {
            throw std::runtime_error("public graph field view is null");
        }
        return value;
    };

    fdb_payload_v1_view_t* root =
        entry_value(opened, UINT32_C(0), UINT64_C(0));
    std::uint32_t component = UINT32_MAX;
    std::uint64_t object_id = UINT64_MAX;
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            root, &component, &object_id, &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));

    component = UINT32_C(77);
    object_id = UINT64_C(88);
    require(fdb_payload_v1_view_graph_identity(
                root, &component, &object_id, nullptr) ==
            FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(component == UINT32_C(77) && object_id == UINT64_C(88));
    component = UINT32_MAX;
    status = fdb_payload_v1_view_graph_identity(
        root, &component, nullptr, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(component == UINT32_C(0) && owned_error(status, error.value));
    error.clear();
    object_id = UINT64_MAX;
    status = fdb_payload_v1_view_graph_identity(
        root, nullptr, &object_id, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(object_id == UINT64_C(0) && owned_error(status, error.value));
    error.clear();
    component = UINT32_MAX;
    object_id = UINT64_MAX;
    status = fdb_payload_v1_view_graph_identity(
        nullptr, &component, &object_id, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(component == UINT32_C(0) && object_id == UINT64_C(0));
    require(owned_error(status, error.value));
    error.clear();

    fdb_payload_v1_view_t* scalar = field(root, UINT32_C(0));
    std::uint8_t u8 = UINT8_C(0);
    require_abi_success(fdb_payload_v1_view_get_bool(
                            scalar, &u8, &error.value),
                        error);
    require(u8 == UINT8_C(1));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(1));
    require_abi_success(fdb_payload_v1_view_get_u8(
                            scalar, &u8, &error.value),
                        error);
    require(u8 == UINT8_C(0x12));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(2));
    std::uint16_t u16 = UINT16_C(0);
    require_abi_success(fdb_payload_v1_view_get_u16(
                            scalar, &u16, &error.value),
                        error);
    require(u16 == UINT16_C(0x3456));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(3));
    std::uint32_t u32 = UINT32_C(0);
    require_abi_success(fdb_payload_v1_view_get_u32(
                            scalar, &u32, &error.value),
                        error);
    require(u32 == UINT32_C(0x789abcde));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(4));
    std::int32_t i32 = INT32_C(0);
    require_abi_success(fdb_payload_v1_view_get_i32(
                            scalar, &i32, &error.value),
                        error);
    require(i32 == INT32_C(-1234567));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(5));
    std::uint64_t u64 = UINT64_C(0);
    require_abi_success(fdb_payload_v1_view_get_u8n_f64_bits(
                            scalar, &u64, &error.value),
                        error);
    require(u64 == UINT64_C(0x3fe0101010101010));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(6));
    require_abi_success(fdb_payload_v1_view_get_u16n_f64_bits(
                            scalar, &u64, &error.value),
                        error);
    require(u64 == UINT64_C(0x3ef0001000100010));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(7));
    require_abi_success(fdb_payload_v1_view_get_f32_bits(
                            scalar, &u32, &error.value),
                        error);
    require(u32 == UINT32_C(0x7fc00000));
    fdb_payload_v1_view_release(scalar);
    scalar = field(root, UINT32_C(8));
    require_abi_success(fdb_payload_v1_view_get_f64_bits(
                            scalar, &u64, &error.value),
                        error);
    require(u64 == UINT64_C(0x7ff8000000000000));
    fdb_payload_v1_view_release(scalar);

    scalar = field(root, UINT32_C(9));
    fdb_payload_v1_access_t* access = nullptr;
    require_abi_success(fdb_payload_v1_view_acquire(
                            scalar, &access, &error.value),
                        error);
    const std::uint8_t* bytes = nullptr;
    std::uint64_t byte_count = UINT64_C(0);
    require_abi_success(fdb_payload_v1_access_str(
                            access, &bytes, &byte_count, &error.value),
                        error);
    require(std::string_view(reinterpret_cast<const char*>(bytes),
                             static_cast<std::size_t>(byte_count)) == "same");
    fdb_payload_v1_access_release(access);
    fdb_payload_v1_view_release(scalar);

    scalar = field(root, UINT32_C(10));
    access = nullptr;
    require_abi_success(fdb_payload_v1_view_acquire(
                            scalar, &access, &error.value),
                        error);
    const std::uint16_t* wide = nullptr;
    std::uint64_t wide_count = UINT64_C(0);
    require_abi_success(fdb_payload_v1_access_wstr(
                            access, &wide, &wide_count, &error.value),
                        error);
    require(wide_count == UINT64_C(3));
    require(wide[0] == UINT16_C(0x0041) && wide[1] == UINT16_C(0xd83d) &&
            wide[2] == UINT16_C(0xde00));
    fdb_payload_v1_access_release(access);
    fdb_payload_v1_view_release(scalar);

    scalar = field(root, UINT32_C(11));
    access = nullptr;
    require_abi_success(fdb_payload_v1_view_acquire(
                            scalar, &access, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_access_bytes(
                            access, &bytes, &byte_count, &error.value),
                        error);
    require(byte_count == UINT64_C(3) && bytes[0] == UINT8_C(0x00) &&
            bytes[1] == UINT8_C(0xff) && bytes[2] == UINT8_C(0x7e));
    fdb_payload_v1_access_release(access);
    fdb_payload_v1_view_release(scalar);

    fdb_payload_v1_view_t* inline_component = field(root, UINT32_C(12));
    component = UINT32_MAX;
    require_abi_success(fdb_payload_v1_view_component_index(
                            inline_component, &component, &error.value),
                        error);
    require(component == inline_component_index);
    component = UINT32_MAX;
    object_id = UINT64_MAX;
    status = fdb_payload_v1_view_graph_identity(
        inline_component, &component, &object_id, &error.value);
    require(status == FDB_PAYLOAD_E_TYPE_MISMATCH);
    require(component == UINT32_C(0) && object_id == UINT64_C(0));
    require(owned_error(status, error.value));
    error.clear();
    scalar = field(inline_component, UINT32_C(0));
    std::uint8_t is_null = UINT8_C(0);
    require_abi_success(fdb_payload_v1_view_is_null(
                            scalar, &is_null, &error.value),
                        error);
    require(is_null == UINT8_C(1));
    fdb_payload_v1_view_release(scalar);
    scalar = field(inline_component, UINT32_C(1));
    require_abi_success(fdb_payload_v1_view_get_u16(
                            scalar, &u16, &error.value),
                        error);
    require(u16 == UINT16_C(0xbeef));
    fdb_payload_v1_view_release(scalar);
    fdb_payload_v1_view_release(inline_component);

    fdb_payload_v1_view_t* list = field(root, UINT32_C(13));
    std::uint64_t length = UINT64_C(0);
    require_abi_success(fdb_payload_v1_view_length(
                            list, &length, &error.value),
                        error);
    require(length == UINT64_C(3));
    fdb_payload_v1_view_t* item = nullptr;
    require_abi_success(fdb_payload_v1_view_at(
                            list, UINT64_C(0), &item, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_view_get_f32_bits(
                            item, &u32, &error.value),
                        error);
    require(u32 == UINT32_C(0x80000000));
    fdb_payload_v1_view_release(item);
    item = nullptr;
    require_abi_success(fdb_payload_v1_view_at(
                            list, UINT64_C(1), &item, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_view_is_null(
                            item, &is_null, &error.value),
                        error);
    require(is_null == UINT8_C(1));
    fdb_payload_v1_view_release(item);
    fdb_payload_v1_view_release(list);

    fdb_payload_v1_view_t* self_ref = field(root, UINT32_C(14));
    component = UINT32_MAX;
    object_id = UINT64_MAX;
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            self_ref, &component, &object_id, &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_t* self_target = nullptr;
    require_abi_success(fdb_payload_v1_view_ref_target(
                            self_ref, &self_target, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            self_target, &component, &object_id, &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_release(self_target);

    require(sweep_handle_allocation_boundary<fdb_payload_v1_view_t>(
                error,
                [self_ref](fdb_payload_v1_view_t** out,
                           fdb_payload_v1_error_t** out_error) {
                    return fdb_payload_v1_view_ref_target(
                        self_ref, out, out_error);
                },
                fdb_payload_v1_view_release) == EXIT_SUCCESS);

    fdb_payload_v1_view_t* asset_ref = field(root, UINT32_C(15));
    fdb_payload_v1_view_t* asset_target = nullptr;
    require_abi_success(fdb_payload_v1_view_ref_target(
                            asset_ref, &asset_target, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            asset_target, &component, &object_id,
                            &error.value),
                        error);
    require(component == asset_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_t* owner_ref = field(asset_target, UINT32_C(1));
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            owner_ref, &component, &object_id, &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_release(owner_ref);
    fdb_payload_v1_view_release(asset_target);
    fdb_payload_v1_view_release(asset_ref);

    fdb_payload_v1_view_t* shared_ref =
        entry_value(opened, UINT32_C(2), UINT64_C(0));
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            shared_ref, &component, &object_id, &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_release(shared_ref);

    fdb_payload_v1_view_t* null_ref =
        entry_value(opened, UINT32_C(2), UINT64_C(1));
    fdb_payload_v1_view_t* null_target =
        reinterpret_cast<fdb_payload_v1_view_t*>(std::uintptr_t{1});
    status = fdb_payload_v1_view_ref_target(
        null_ref, &null_target, &error.value);
    require(status == FDB_PAYLOAD_E_UNEXPECTED_NULL);
    require(null_target == nullptr && owned_error(status, error.value));
    error.clear();
    component = UINT32_MAX;
    object_id = UINT64_MAX;
    status = fdb_payload_v1_view_graph_identity(
        null_ref, &component, &object_id, &error.value);
    require(status == FDB_PAYLOAD_E_UNEXPECTED_NULL);
    require(component == UINT32_C(0) && object_id == UINT64_C(0));
    require(owned_error(status, error.value));
    error.clear();
    fdb_payload_v1_view_release(null_ref);

    fdb_payload_v1_view_t* cleared_target =
        reinterpret_cast<fdb_payload_v1_view_t*>(std::uintptr_t{1});
    status = fdb_payload_v1_view_ref_target(root, nullptr, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(owned_error(status, error.value));
    error.clear();
    status = fdb_payload_v1_view_ref_target(
        nullptr, &cleared_target, &error.value);
    require(status == FDB_PAYLOAD_E_INVALID_ARGUMENT);
    require(cleared_target == nullptr && owned_error(status, error.value));
    error.clear();

    fdb_payload_v1_view_t* detached = nullptr;
    require_abi_success(fdb_payload_v1_view_materialize(
                            root, &detached, &error.value),
                        error);
    require(detached != nullptr);
    require_abi_success(fdb_payload_v1_payload_invalidate(
                            opened, &error.value),
                        error);
    std::uint32_t kind = UINT32_MAX;
    status = fdb_payload_v1_view_kind(root, &kind, &error.value);
    require(status == FDB_PAYLOAD_E_VIEW_INVALIDATED);
    require(kind == UINT32_C(0) && owned_error(status, error.value));
    error.clear();
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            detached, &component, &object_id, &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_t* detached_self = field(detached, UINT32_C(14));
    fdb_payload_v1_view_t* detached_target = nullptr;
    require_abi_success(fdb_payload_v1_view_ref_target(
                            detached_self, &detached_target, &error.value),
                        error);
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            detached_target, &component, &object_id,
                            &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_release(detached_target);
    fdb_payload_v1_view_release(detached_self);
    fdb_payload_v1_view_release(detached);
    fdb_payload_v1_view_release(self_ref);
    fdb_payload_v1_view_release(root);
    fdb_payload_v1_payload_release(opened);

    BackingContext external_context;
    external_context.bytes.assign(
        fdb_payload_v1_blob_data(binary),
        fdb_payload_v1_blob_data(binary) + fdb_payload_v1_blob_size(binary));
    auto external_backing = backing_for(external_context);
    fdb_payload_v1_payload_t* external = nullptr;
    require_abi_success(fdb_payload_v1_payload_open_external(
                            spec, external_context.bytes.data(),
                            external_context.bytes.size(), &external_backing,
                            &external_context, nullptr, &external,
                            &error.value),
                        error);
    require(external_context.retains == UINT32_C(1));
    fdb_payload_v1_view_t* external_root =
        entry_value(external, UINT32_C(0), UINT64_C(0));
    require_abi_success(fdb_payload_v1_view_graph_identity(
                            external_root, &component, &object_id,
                            &error.value),
                        error);
    require(component == node_component && object_id == UINT64_C(0));
    fdb_payload_v1_view_release(external_root);
    require_abi_success(fdb_payload_v1_payload_invalidate(
                            external, &error.value),
                        error);
    require(external_context.releases == UINT32_C(1));
    fdb_payload_v1_payload_release(external);
    require(external_context.releases == UINT32_C(1));
    ...
    require_abi_success(fdb_payload_v1_payload_invalidate(
                            heap_payload, &error.value),
                        error);
    fdb_payload_v1_payload_release(heap_payload);
    fdb_payload_v1_blob_release(binary);
    fdb_payload_v1_plan_release(plan);
    fdb_payload_v1_spec_release(spec);
    return EXIT_SUCCESS;
...
#define REQUIRE_NULL_SINK_REJECTED(...)                                      \
    ...
    fdb_payload_v1_object_handle_t object = UINT64_C(0xfeed);
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_builder_object_declare(
        nullptr, UINT32_C(0), &object, nullptr));
    require(object == UINT64_C(0xfeed));
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_builder_object_fill_begin(
        nullptr, UINT64_C(1), nullptr));
    REQUIRE_NULL_SINK_REJECTED(
    ...
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_builder_value_list_begin(
        nullptr, UINT64_C(0), nullptr));
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_builder_value_object(
        nullptr, UINT64_C(1), nullptr));
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_builder_value_ref(
        nullptr, UINT64_C(1), nullptr));
    ...
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_view_field(
        nullptr, UINT32_C(0), &view, nullptr));
    REQUIRE_NULL_SINK_REJECTED(
        fdb_payload_v1_view_ref_target(nullptr, &view, nullptr));
    require(view ==
            reinterpret_cast<fdb_payload_v1_view_t*>(std::uintptr_t{1}));
    REQUIRE_NULL_SINK_REJECTED(fdb_payload_v1_view_graph_identity(
        nullptr, &view_u32, &view_u64, nullptr));
    require(view_u32 == UINT32_MAX && view_u64 == UINT64_MAX);
    REQUIRE_NULL_SINK_REJECTED(
...
int test_errors_prefixes_and_callbacks() {
    ErrorRef error;
    fdb_payload_v1_builder_t* builder =
        reinterpret_cast<fdb_payload_v1_builder_t*>(std::uintptr_t{1});
...
int main(int argc, char** argv) {
    ...
    require(test_initializers_and_constants() == EXIT_SUCCESS);
    require(test_graph_builder_option_prefixes() == EXIT_SUCCESS);
    require(test_guarded_v1_prefixes_do_not_touch_v2_tails() == EXIT_SUCCESS);
    require(test_public_graph_handle_errors() == EXIT_SUCCESS);
    ...
    require(test_errors_prefixes_and_callbacks() == EXIT_SUCCESS);
    return EXIT_SUCCESS;
}
