...
SlotLayout ordinary_slot(TypeKind kind) noexcept {
    switch (kind) {
...
SlotLayout topology_slot(TypeKind kind, StorageRole role) noexcept {
    switch (role) {
    case StorageRole::ordinary_value:
        return ordinary_slot(kind);
    case StorageRole::list_descriptor:
        return {UINT32_C(16), UINT32_C(8)};
    case StorageRole::object_root_id:
    case StorageRole::reference_id:
        return {UINT32_C(8), UINT32_C(8)};
    case StorageRole::inline_component:
        return {UINT32_C(0), UINT32_C(0)};
    }
    return {UINT32_C(0), UINT32_C(0)};
}
...
Result<RuntimeSchema> RuntimeSchema::compile(
    const spec::CompiledSpec& compiled) {
    auto topology = spec::derive_runtime_topology(compiled.resolved());
    if (!topology.has_value()) {
        return Result<RuntimeSchema>::failure(std::move(topology).error());
    }
    RuntimeSchema schema(compiled);
    auto assigned = schema.assign_topology(std::move(topology).value());
...
Result<void> RuntimeSchema::assign_topology(RuntimeTopology topology) {
    ...
    for (std::size_t index = 0U; index < topology.types.size(); ++index) {
        ...
        if (source.source == nullptr ||
            ...
            return Result<void>::failure(runtime_error(
                ...
                "Runtime topology contains an invalid source type",
        ...
        if (!inserted.second) {
            return Result<void>::failure(runtime_error(
                JsonPointer{}.append("runtime").append("types"),
                FDB_PAYLOAD_E_INTERNAL,
                "Runtime topology contains a duplicate source type",
                "duplicate_source_type"));
        }
        types_.push_back(RuntimeType{
            id, source.source,
    ...
    if (reachable_components_.size() !=
            spec_.resolved().components().size() ||
        identity_component_flags_.size() !=
            spec_.resolved().components().size()) {
        return Result<void>::failure(runtime_error(
            JsonPointer{}.append("runtime").append("components"),
            FDB_PAYLOAD_E_INTERNAL,
            "Runtime topology component inventory has an invalid size",
            "invalid_runtime_topology"));
    }
    identity_components_.reserve(identity_component_flags_.size());
    for (std::size_t index = 0U;
         index < identity_component_flags_.size(); ++index) {
        if (identity_component_flags_[index] != UINT8_C(0)) {
            identity_components_.push_back(
...
Result<void> RuntimeSchema::compile_component_layouts() {
    ...
    while (next_ready < ready.size()) {
        ...
        for (const spec::Field& field : component.fields) {
            ...
            const RuntimeType* const field_runtime =
                find_type(field_type_id);
            if (field_type_id == UINT32_MAX || field_runtime == nullptr) {
                return Result<void>::failure(runtime_error(
                    JsonPointer{}
                        .append("components")
                        .append(component.id)
            ...
            SlotLayout slot = field_runtime->slot;
            if (field.type.kind == TypeKind::component) {
