...
For C-Two and other RPC runtimes, fastdb behaves as a provider-owned payload codec family: fastdb exports schema identity and adapters through `fastdb4py.schema` and `fastdb4py.c_two_provider`, while the runtime records an opaque codec reference and invokes encode/decode hooks without understanding fastdb internals.
