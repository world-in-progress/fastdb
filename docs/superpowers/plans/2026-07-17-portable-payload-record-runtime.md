...
- An unsafe raw-span API. Scoped access is the P2 zero-copy contract; adding a deliberately unrevocable native escape hatch is optional future API work and is not required by the accepted success criteria.
...
`layout::RuntimeSchema::compile(const spec::CompiledSpec&)` assigns a finite `uint32_t` runtime type ID to every source `TypeNode` without expanding component DAGs:

1. Visit entries by stable entry index. Assign the entry root node, then recurse only through `list.items`; a `component` node records its resolved component index but does not inline-visit that component's fields.
2. Visit components by stable component index and fields by stable field index. Assign each field root node and recurse only through `list.items`.
3. A reachable component layout references the already assigned field root IDs. Record-profile reachability begins at entries and follows by-value component/list edges iteratively.
4. `ref` in a record runtime schema is an internal invariant failure because P1 rejects it for `record.v1`; `object_graph.v1` is rejected before runtime-schema creation with `RUNTIME_UNAVAILABLE`.

This produces stable IDs without exponential by-value expansion. Runtime type IDs are binary-local validation facts; they do not change canonical JSON, payload digest, or P1 stable entry/component/field indexes.
...
`begin_entry` accepts entries in any order exactly once. It validates `one -> value_count == 1`; `many` accepts zero or more. Value operations consume the next expected type in prefix order. `begin_component` pushes its fields in field-index order and `begin_list(count)` pushes exactly `count` item expectations; frames auto-close when their declared children are complete, including empty components/lists. No end-call ambiguity exists.
...
Entry-region owner indexes are stable entry indexes. List-region owner indexes are the list node's stable runtime type ID and the descriptor type ID is its item type ID. Pool owner/type IDs are `UINT32_MAX`.
...
A zero-length region is still present when required by the canonical inventory. Its offset is the aligned current end; adjacent zero-length regions may share that boundary, but neither can point inside a non-empty region or create an optional gap.
...
- `str`, `wstr`, and `bytes` use the one corresponding global pool. Non-null values append their exact bytes in canonical logical traversal order with no terminator, deduplication, aliasing, or unrequired padding.
- `wstr` pool offsets and lengths are bytes; both must be even. The bytes are well-formed UTF-16LE and the checked view reports `uint16_t` code units.
- A non-null empty variable value stores the pool's current append offset and length zero. A null value stores two zero words.
- Each reachable list type node owns one aggregated LIST_ITEMS region and optional LIST_VALIDITY region. A non-null list descriptor stores the current item count for that list node and its declared count. A null list stores two zero words.
- In canonical parent traversal, non-null list descriptors must partition their item region in order without gaps, overlap, or aliasing. Pool descriptors likewise partition each global pool exactly in canonical traversal order.
...
The committed/external readable base must be aligned to 8 bytes. `open_copy` obtains that alignment from Core heap backing; `open_external` rejects a misaligned caller base at `/binary/base` with `MISALIGNED`. This makes the checked `uint16_t` wide-string span valid in C while all wire loads still use unaligned-safe byte helpers.

Each structural header/directory/value/list item costs one validation-work unit; each inspected text byte/code-unit costs one additional unit. Every increment is checked before comparison. Safe zero-selecting defaults are:
...
reserve -> writes -> failure -> rollback
reserve -> writes -> commit failure -> rollback
```

After a successful reserve, exactly one rollback occurs on every pre-commit/commit failure and none after commit success. Commit is atomic: failure leaves the reservation uncommitted and rollback-capable. If rollback also fails, `ROLLBACK_FAILED` is returned with the original code/symbol and rollback status in canonical details. Commit success returns the exact contiguous readable base/length and transfers the reservation's initial owner reference to `PayloadOwner`.
...
A checked view retains shared owner state and captures its generation. Scalar/navigation calls acquire a short internal pin. Borrowed payload/string/wstring/bytes pointers require an explicit unique `Access` pin. Pin acquisition under the mutex succeeds only when the owner is neither invalidating nor invalidated and the captured generation matches. The pin increments `active_accesses`; release decrements and wakes invalidation when it reaches zero.
...
`fdb_payload_v1_backing_init` assigns each pointer/function-pointer member to C `NULL` explicitly rather than assuming an all-bits-zero pointer representation; it zeroes only integer reserved fields and sets the current target's `sizeof` prefix.
...
error: RUNTIME_UNAVAILABLE=2009
```
...
This is 64 additive exports and 97 total `fdb_payload_v1_*` symbols after P2. Task 9 must update the exact sorted allowlist to 97 lines; a count-only check remains forbidden.
...
**Interfaces produced:** the exact `ValueNode`, `BuilderLimits`, `LogicalPayload`, and internal `PayloadBuilder` contract above; reusable strict UTF-8/UTF-16 validation; exact binary64 finite/range validation seam for later quantization; additive error constant/symbol `2009 RUNTIME_UNAVAILABLE`.
...
- [ ] Add RED tests that compile explicit record specs and exercise `PayloadBuilder` directly:
  ...
  - custom limits for nodes/list elements/text/bytes/depth/total bytes before allocation growth;
...
- [ ] Make every authoring operation transactional: validate expected type/null/range/text/limits and reserve all required vector/byte capacity before publishing node/link/count changes. Allocation failure leaves the builder valid and retryable.
...
**Interfaces produced:** `RuntimeSchema`, checked wire primitives, canonical `RecordLayout`, one bounded `ByteSink` writer, initial strict `open_record`, and the first independently reviewable bytes locking header/directories/scalar slots.

- [ ] Add RED layout tests for stable runtime type IDs, finite component DAG reuse, unreachable component exclusion, field offsets/alignments/stride, empty-component stride one, nullable bitmap positions, list-node inventory, and checked arithmetic at every `uint32_t`/`uint64_t` boundary.
- [ ] Add RED binary tests for exact empty/fixed-scalar golden hex, SHA-256 of decoded golden bytes, build determinism across repeated builders/plans, header/directory field offsets, zero padding, spec-digest embedding, signed zero/infinity/canonical NaN, and open observation of the same values.
...
- [ ] Implement runtime type-ID assignment and iterative reachable-component/type analysis exactly as frozen above. Cache reusable component layouts by stable component index; reject an internal by-value cycle/ref rather than recurse.
...
- [ ] Write the normative Markdown document in the same commit. Include every offset, region order, component/validity rule, pool/list rule, numeric canonicality rule, overflow/alignment rule, open behavior, resource-limit behavior, and forward-compatibility rejection stated in this plan; link the design/ADR/issues and golden index.
...
- [ ] Extend open with partition cursors per pool and eager/lazy validation flags. Lazy mode stores validated descriptor bounds but validates text contents under each later access pin; bytes never receive text validation.
...
- [ ] Build a table-driven fake backing that records every callback argument and can inject failure at reserve/write N/commit/rollback/retain. Add RED cases for:
  ...
  - short capacity, no writable span and no write callback, partial/overlapping write behavior, wrong commit base/size, and non-zero callback status;
  - a null owner-token value accepted and passed unchanged through commit/release;
  - reserve/write/commit failure followed by exactly one rollback;
  - rollback failure details preserving the original failure;
  - no rollback after commit success and exactly one final release of the committed owner token;
...
- [ ] Implement `BackingReservation` as an explicit state machine (`empty,reserved,committed,rolled_back`) whose destructor attempts rollback only for a live uncommitted reservation. It must never call user callbacks while holding unrelated Core locks.
...
- [ ] On commit success, validate non-null readable base for non-zero size, exact used length, capacity, and token. Transfer rather than duplicate the reservation reference.
- [ ] Update Issue 0002: plan/backing execution is internally complete and truthful, while open publication, views, lifetime barrier, and C/C++ APIs remain unavailable.
...
- [ ] Add ownership RED cases:
  ...
  - committed backing corruption/wrong commit read span fails as `BACKING_CONTRACT` and publishes no owner.
...
- [ ] For external open, validate callback prefix/retain/release, call retain, install an RAII release guard, then validate bytes. Publish the shared state only after success; dismiss the guard into owner state.
...
- [ ] Map every malformed error to `/binary/header/...`, `/binary/regions/{i}/...`, `/binary/entries/{i}/...`, or the logical entry/field/list path. Details include exact expected/actual values without platform addresses.
...
- [ ] Add RED view matrix for every root/component/list/scalar/text/bytes context: kind, null, length, index bounds, field count/index, component index, exact scalar bits, normalized decode, and variable spans. Null getters return `UNEXPECTED_NULL`; wrong getters return `TYPE_MISMATCH`; index failures return `INDEX_OUT_OF_RANGE`.
...
- [ ] Implement scalar/navigation calls with an internal short pin. Implement `Access` as the only checked borrowed-span carrier; it owns the active count until release and exposes only the span kind it was acquired for.
...
- [ ] Add RED ABI tests for every Task 9 function, null/error-sink rule, span null/zero pairs, options future tails/canaries, flags/reserved rejection, output clearing, object-graph runtime unavailable, builder unique lifetime/state, plan atomic retain/release/concurrent queries, callback ordering/failures, external-open retain/release, opened execution-report absence, and invalidation/release balance.
- [ ] Add static assertions for 88/112/104/72 pointer-free struct sizes on arm64/x86_64/wasm32 C11 compilation. For the callback table, assert field order with `offsetof`, initializer `struct_size == sizeof`, and prefix acceptance on the current target rather than inventing a cross-pointer-size constant.
...
- [ ] Add the Task 9 subset of the frozen declarations without changing any P1 prefix/function. Validate every struct before reading limits/callbacks, copy the known callback prefix into Core state, and preserve larger caller tails/canaries.
...
- [ ] Project all typed builder operations without conversion beyond exact bits/spans. `builder_freeze` publishes a plan only after complete success and preserves the retryable builder on ordinary failure.
...
- [ ] Update record manifests/capabilities only for public compile/query/build/open/invalidate/direct-eligible behavior available at this commit. Object-graph manifests stay compile/query/not-evaluated. Regenerate every affected spec manifest golden without changing canonical payload/digest goldens.
- [ ] Update the sorted symbol allowlist to the exact intermediate declarations and require zero other additions/removals. Keep the final expected 97-symbol target for Task 10.
...
**Interfaces produced:** the complete frozen 97-symbol P2 C ABI and thin `Builder`, `BuildPlan`, `Payload`, `View`, `Access`, options/info/report RAII facade with no semantic duplication.

- [ ] Add RED C ABI tests for all navigation/scalar getters, null/type/index errors, view atomic lifetime, scoped payload/text/wtext/bytes access, pointer borrow duration, detached materialization, invalidate blocking on an access handle, stale-view failure, and output clearing/allocation failure.
...
- [ ] Confirm RED and allowlist mismatch, then add the remaining frozen functions/opaque access/view handles. View handles use saturating atomic references. Access handles uniquely own either one active owner pin or a detached-arena pin and release it exactly once.
- [ ] Make every checked borrowed pointer obtainable only from a live `Access`; no view function returns a backing pointer. `access_payload_bytes` accepts only payload access, and str/wstr/bytes accessors accept only the corresponding value access.
...
- [ ] Update the exact sorted ABI allowlist to 97 unique lines and independently diff declared, defined, and exported names on native builds.
...
- [ ] Run pure-C compile/link, runtime ABI, C++ facade parity, exact 97-symbol gate, 32-thread immutable handle/view queries, active-pin barrier tests, Debug/Release/ASan+UBSan, ThreadSanitizer where available, Python/TypeScript/WASM regressions, and `git diff --check`.
- [ ] Commit:
...
- [ ] Add/extend CI so standard `ubuntu-24.04` x64 and `macos-15` arm64 native jobs run the complete P2 suite, exact 97-symbol check, binary corpus/hash checks, and C/C++ ABI sizes. Linux sanitizer CI runs both spec-compile and binary-open robustness smokes. The aggregate must distinguish expected path skips from required-job failure/cancellation exactly as P1 does.
...
- [ ] Add a complete P2 proof map from each accepted design Section 8-12/18 requirement and goal P2 bullet to exact test names/goldens. Explicitly name all non-`ref` types and all entry/component/list contexts.
...
- [ ] Run the complete clean local gate from new directories:
  ...
  Expected: every available local command passes with exact counts recorded; sanitizers are clean; exact ABI is 97; every valid/malformed golden matches; package builds succeed. Hosted jobs remain expected, not locally passed.
...
- [ ] Commit the hardening/docs change:
  ...
  ```bash
  git add tests/fuzz/payload tests/cpp/payload tests/cpp/CMakeLists.txt \
...
7. Preserve ignored `.superpowers/sdd/task-<n>-report.md` with exact RED/GREEN commands, platform/toolchain, counts, artifacts, limitations, implementation/review/fix commits, and reviewer result.

## Final P2 Review Gate

- [ ] Trace every goal P2 deliverable and accepted design Sections 8-12, 18.2, 18.4, 18.5, 18.7, and 20 step 2-3 to an exact implementation symbol plus test/golden.
...
- [ ] Confirm exact 97-symbol ABI and all fixed struct sizes/future-tail canaries on available targets; hosted target results are cited only when an authorized run exists.
